...
using UnityEngine;
using Ardunity;
public class picking3 : MonoBehaviour
{
    Ray m_ray;
    RaycastHit m_rayHit;
    ...
    void Start()
    {
        lt3.gameObject.GetComponent<Light>().enabled = false;
    }
    ...
    void Update()
    {
        if (Input.GetMouseButtonDown(0))
        {
            ...
            if (m_selectObject != null && count == 0 && m_selectObject.name == "Lamp 3")
            {
                cr.DoTriggerEnter();
                lt3.gameObject.GetComponent<Light>().enabled = true;
                count = 1;
                temp = 1;
            }
            if (m_selectObject != null && count == 1 && temp == 0 && m_selectObject.name == "Lamp 3")
            {
                m_selectObject = null;
                cr.DoTriggerExit();
                lt3.gameObject.GetComponent<Light>().enabled = false;
