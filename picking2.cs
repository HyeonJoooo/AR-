...
public class picking2 : MonoBehaviour
{
    ...
    void Start()
    {
        lt2.gameObject.GetComponent<Light>().enabled = false;
    }
    ...
    void Update()
    {
        if (Input.GetMouseButtonDown(0))
        {
            ...
            if (m_selectObject != null && count == 0 && m_selectObject.name == "Lamp 2")
            {
                cr.DoTriggerEnter();
                lt2.gameObject.GetComponent<Light>().enabled = true;
                count = 1;
                temp = 1;
            }
            if (m_selectObject != null && count == 1 && temp == 0 && m_selectObject.name == "Lamp 2")
            {
                m_selectObject = null;
                cr.DoTriggerExit();
                lt2.gameObject.GetComponent<Light>().enabled = false;
